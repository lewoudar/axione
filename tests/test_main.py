...
def test_should_return_empty_list_if_criteria_does_not_match_any_row_in_apartment_file(client, mocker, payload, caplog):
    caplog.set_level(logging.INFO, logger='axione')
...
def test_should_return_correct_payload_given_correct_input(client, respx_mock, mock_http_calls, settings, caplog):
    caplog.set_level(logging.DEBUG, logger='axione.main')
    ...
    assert 'fetching city information' in caplog.text
    assert 'caching and returning 49 items found' in caplog.text
    ...
    assert 'returning data in cache for identifier' in caplog.text
